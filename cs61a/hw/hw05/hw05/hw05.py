...
def balanced(m):
    ...
    if is_weight(m):
        return True
    elif not balanced(end(sides(m)[0])):
        return False
    else:
        if not balanced(end(sides(m)[1])):
            return False
        else:
            s1 = sides(m)[0]
            s2 = sides(m)[1]
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attempts = []
    def withdraw_correct_pass(amount, response):
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    called_withdraw = withdraw(0, old_password)
    def check_withdraw(amount, response):
        if response ==  new_password:
            response = old_password
        return withdraw(amount, response)
    if type(called_withdraw) == int:
        return check_withdraw
    else:
        return called_withdraw
...
class VendingMachine:
    ...
    def restock(self, stock_amount):
        ...
        return 'Current ' + self.thing + ' stock: %d' %self.stock
    ...
    def vend(self):
        if self.stock > 0:
            if self.balance == self.cost:
                ...
                self.balance = 0
                self.stock = self.stock - 1
                return 'Here is your ' + self.thing + ' and $%d change.' %change
            else:
                missing = self.cost - self.balance
                return 'You must deposit $%d more.' %missing
        else:
            if self.balance == 0:
...
class MissManners:
    ...
    def ask(self,command,*arg):
        if command[:6]=='please':
            if hasattr(self.other_obj, command[7:]):
                return getattr(self.other_obj, command[7:])(*arg)
            else:
                return 'Thanks for asking, but I know not how to ' + command[7:] + '.'
        else:
