...
def do_and_form(expressions, env):
    """Evaluate a short-circuited and"""
    # END Question 14B
    if expressions is nil:
        return True
    else:
        while expressions != nil:
            before = scheme_eval(expressions.first, env)
            if not scheme_true(before):
                return False
            else:
                expressions = expressions.second
        return before
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_symbolp(current.first):
            ...
            parameter = Pair(current.first, parameter)
            value = Pair(scheme_eval(current.second.first, env), value)
            bindings = bindings.second
    return env.make_child_frame(parameter, value)
    # END Question 16
...
def check_formals(formals):
    ...
    repeated = []
    while formals != nil:
        if not isinstance(formals.second, Pair) and formals.second is not nil:
            raise SchemeError
        elif formals.first in repeated:
            raise SchemeError
