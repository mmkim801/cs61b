...
(+ 5 (begin
            ...
            (newline)
            (* 6 (/ 48 7 (+ 5 8 (- 5 8))) 5 4)))
; expect 87.28571428571428

((lambda (c d) (begin (print c) (print d))) 1995 1)
; expect okay

((lambda (a) (+ 5 a 150)) 15 3)
; expect Error

((lambda (i m n) (* i (+ m n))) 1 5 7)
; expect 12

((eval (define (half s) (/ s 2))) 16)
; expect 8

(define (square x) (* x x))
(eval (define (make-even a b) (+ (square a) (square b))))
; expect (lambda (a b) (+ (square a) (square b)))

(make-even 0 5 7)
; expect Error

(cond ((even? 4) 'almost-done)
      ((zero? 4) 'wrong)
      (else False)
)
; expect almost-done

(define (func x) (+ x (- (* 2 x) x)))
(cond ((> (func 3) 10) 'are-you-sure?)
      ((< (func 3) 10))
      (else 'repeat))
; expect True

(cond ((eq? 'hi 'hello) 'stupid)
      ((eq? 'love 'love) 'lovebirds)
      ((4 > 0) 'sup)
      (else False)
  )
; expect lovebirds
