...
def abundant(n):
    ...
    total = 0
    factor = 1
    while factor <= n and factor <= n/factor:
        if n % factor == 0:
            print (factor, '*', n//factor)
            if factor != n//factor:
                total = total + factor + n//factor
            else:
                total = total + factor
        factor = factor + 1
...
def amicable(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def sum_of_factors(q):
        result = 0
        factor = 1
        while factor <= n**(1/2):
            if q % factor == 0:
                if factor != q//factor:
                    result = result + factor + q//factor
                else:
    ...
    m=n+1
    while sum_of_factors(sum_of_factors(m))!=m or sum_of_factors(m)==m:
        m+=1
